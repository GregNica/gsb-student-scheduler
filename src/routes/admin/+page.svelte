<script lang="ts">
	...
	const TIME_OPTIONS: string[] = (() => {
		const t: string[] = [];
		for (let h = 8; h <= 21; h++) for (let m = 0; m < 60; m += 5) t.push(`${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}`);
		return t;
	})();
	...
	function snapTo5(time: string): string {
		...
		return `${String(snapped >= 60 ? h + 1 : h).padStart(2,'0')}:${String(snapped >= 60 ? snapped - 60 : snapped).padStart(2,'0')}`;
	}
	...
	function sortRows(rows: EditableRow[]): EditableRow[] {
		return [...rows].sort((a, b) => {
			const aDay = DAY_ORDER[DAY_OPTIONS.find(o => o.label === a.days)?.days[0] ?? ''] ?? 99;
			const bDay = DAY_ORDER[DAY_OPTIONS.find(o => o.label === b.days)?.days[0] ?? ''] ?? 99;
	...
	function courseToRow(course: ParsedCourse): EditableRow {
		...
		const days = matchDayOption([...new Set(slots.map(s => s.day))]);
		...
		return {
			...
			days, timeBlocks, room: [...new Set(slots.map(s => s.room).filter(Boolean))].join(', '),
	...
	function rowToCourse(row: EditableRow): ParsedCourse {
		...
		for (const day of dayNames) for (const block of row.timeBlocks) {
			if (block.startTime && block.endTime) meetingSlots.push({ day, dayCode: day.slice(0, 3), startTime: block.startTime, endTime: block.endTime, room: row.room });
		}
		return {
	...
	async function saveToDatabase() {
		isSaving = true; saveSuccess = ''; parseError = '';
		try {
			const semesters = editableGroups.map(g => ({ period: g.period, courses: g.rows.map(rowToCourse) }));
			...
			saveSuccess = `Saved ${updated} semester${updated !== 1 ? 's' : ''} to the database. Students will see the updated schedule immediately.`;
			step = 'done';
		} catch (e) { parseError = e instanceof Error ? e.message : 'Save failed'; }
		finally { isSaving = false; }
	}

	function reset() { uploadedFile = null; editableGroups = []; unmatchedCourses = []; parseError = ''; saveSuccess = ''; step = 'upload'; }
...
<div class="space-y-6">
	<div class="border-b pb-4">
		<h1 class="text-xl font-bold">Course Upload</h1>
		<p class="text-sm text-muted-foreground mt-1">Parse PDF timetables and save course data to the database.</p>
	</div>
	...
	{#if step === 'upload'}
		<div class="space-y-6">
			...
			<div class="space-y-3">
				...
				<p class="text-xs text-muted-foreground">Select all periods covered by the PDF. The parser will assign each course to its matching period automatically.</p>
				<div class="space-y-4">
								...
								{#each periods as period}
									{#if editingPeriodId === period.id && editDraft}
										<div class="col-span-2 p-3 rounded-lg border border-primary/50 bg-primary/5 space-y-2">
											...
											<div class="grid grid-cols-2 gap-2">
												<div><label class="text-xs font-medium block mb-0.5" for="el">Label</label><input id="el" bind:value={editDraft.label} class="w-full border rounded px-2 py-1 text-xs bg-background" /></div>
												<div><label class="text-xs font-medium block mb-0.5" for="ec">Category</label><input id="ec" bind:value={editDraft.category} class="w-full border rounded px-2 py-1 text-xs bg-background" /></div>
												<div class="col-span-2"><label class="text-xs font-medium block mb-0.5" for="ed">Description</label><input id="ed" bind:value={editDraft.description} class="w-full border rounded px-2 py-1 text-xs bg-background" /></div>
												<div><label class="text-xs font-medium block mb-0.5" for="es">Start date</label><input id="es" type="date" bind:value={editDraft.startDate} class="w-full border rounded px-2 py-1 text-xs bg-background" /></div>
												<div><label class="text-xs font-medium block mb-0.5" for="ee">End date</label><input id="ee" type="date" bind:value={editDraft.endDate} class="w-full border rounded px-2 py-1 text-xs bg-background" /></div>
												<div><label class="text-xs font-medium block mb-0.5" for="es2">Start date 2 <span class="text-muted-foreground font-normal">(optional)</span></label><input id="es2" type="date" bind:value={editDraft.startDate2} class="w-full border rounded px-2 py-1 text-xs bg-background" /></div>
												<div><label class="text-xs font-medium block mb-0.5" for="ee2">End date 2 <span class="text-muted-foreground font-normal">(optional)</span></label><input id="ee2" type="date" bind:value={editDraft.endDate2} class="w-full border rounded px-2 py-1 text-xs bg-background" /></div>
											</div>
											<label class="flex items-center gap-2 text-xs cursor-pointer"><input type="checkbox" bind:checked={editDraft.tentative} /> Mark as tentative</label>
									...
									{:else}
										<div class="flex items-start gap-2 p-2.5 rounded-lg border transition-colors {selectedSemesterIds.includes(period.id) ? 'border-primary bg-primary/5' : 'border-muted hover:border-muted-foreground/40'}">
											<label class="flex items-start gap-2 flex-1 cursor-pointer min-w-0">
												<input type="checkbox" class="mt-0.5 accent-primary shrink-0" checked={selectedSemesterIds.includes(period.id)} onchange={(e) => toggleSemester(period.id, (e.target as HTMLInputElement).checked)} />
												<div class="min-w-0">
				...
				{#if selectedSemesterIds.length > 0}<p class="text-xs text-green-700 font-medium">✓ Selected: {selectedLabels}</p>{/if}
			</div>
		...
		<div class="space-y-6">
			...
			{#if unmatchedCourses.length > 0}
				<div class="bg-amber-50 border border-amber-200 rounded-lg p-3 text-sm text-amber-800 space-y-1">
					<p><strong>{unmatchedCourses.length} course{unmatchedCourses.length !== 1 ? 's' : ''} could not be matched to a selected period</strong> and will not be saved:</p>
			...
			{#each editableGroups as group, groupIdx}
				<div class="space-y-2">
					...
					{#if group.rows.length === 0}
						<p class="text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">No courses matched this period. Use "+ Add course" to add manually.</p>
					{/if}
							...
							<tbody>
								{#each group.rows as row, rowIdx}
									<tr class="border-t {rowIdx % 2 === 0 ? '' : 'bg-muted/20'} align-top">
										<td class="px-2 py-1.5"><button onclick={() => removeRow(groupIdx, rowIdx)} class="text-xs text-red-500 hover:text-red-700 border border-red-300 rounded px-2 py-0.5 font-medium">Delete</button></td>
										...
										<td class="px-1 py-1.5"><select bind:value={row.program} class={sel}><option value="FMBA">FMBA</option><option value="MMS">MMS</option><option value="unknown">—</option></select></td>
										<td class="px-1 py-1.5"><input bind:value={row.instructor} class={inp} placeholder="Name" /></td>
										<td class="px-1 py-1.5"><select bind:value={row.days} class={sel}><option value="">— select —</option>{#each DAY_OPTIONS as opt}<option value={opt.label}>{opt.label}</option>{/each}</select></td>
										<td class="px-1 py-1.5 space-y-1">
											{#each row.timeBlocks as block, blockIdx}
												<div class="flex items-center gap-1">
													<select bind:value={block.startTime} class="{sel} w-20"><option value="">start</option>{#each TIME_OPTIONS as t}<option value={t}>{t}</option>{/each}</select>
													<span class="text-muted-foreground shrink-0">–</span>
													<select bind:value={block.endTime} class="{sel} w-20"><option value="">end</option>{#each TIME_OPTIONS as t}<option value={t}>{t}</option>{/each}</select>
													{#if row.timeBlocks.length > 1}<button onclick={() => removeBlock(groupIdx, rowIdx, blockIdx)} class="shrink-0 text-muted-foreground hover:text-red-500 font-bold px-0.5">−</button>{/if}
												</div>
											{/each}
			...
			<div class="flex gap-3 pt-2">
				...
				<button onclick={saveToDatabase} disabled={isSaving}
					class="flex-1 bg-green-600 text-white rounded-lg px-4 py-2.5 text-sm font-medium hover:bg-green-700 disabled:opacity-50">
	...
	{:else if step === 'done'}
		<div class="space-y-4">
			<div class="bg-green-50 border border-green-200 rounded-lg p-4 text-sm text-green-800">
				<strong>Saved successfully!</strong> {saveSuccess}
			</div>
